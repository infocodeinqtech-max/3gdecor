...
function ContactHero({ content }: { content: ContactPageContent }) {
  ...
  return (
    <section
      // className="bg-[#F5F1EA] px-4 lg:px-5 overflow-x-hidden"
      className="
        bg-[#F5F1EA]
        px-4
        lg:px-5
...
export default function Contact() {
  ...
  const { showLoader, fading } = useCmsPageGate(async (force) => {
    ...
    const rows = (site.contactOffices as ContactOffice[] | undefined)?.length
                  ...
                  >
                    ...
                    <div className="relative z-[1]">
                      ...
                      <p className="mt-2 text-sm text-white/65 leading-relaxed">
                        We sent a 6-digit OTP to your provided email. Enter it
                        below to submit your inquiry.
                      </p>
                      <p className="mt-1 text-[12px] text-[#f3bb27]/90">
                        OTP expires in{" "}
                        {Math.max(1, Math.ceil(otpExpiresIn / 60))} minutes.
                      </p>
